...
function checkAuth() {
  ...
  const storedUser = sessionUser && sessionUser.role === "hr"
    ? JSON.stringify(sessionUser)
    : localStorage.getItem("hrms_hr_user");
  const loginScreen = document.getElementById("login-screen");
  
  if (storedUser) {
    currentUser = JSON.parse(storedUser);
    localStorage.setItem("hrms_hr_user", JSON.stringify(currentUser));
    if (loginScreen) {
...
function getHrUsers() {
  let users = localStorage.getItem("hrms_registered_hr");
  if (!users) {
    users = [
      { name: "Elena Vance", email: "hr@sundew.com", password: "password123", role: "Lead HR Manager", avatar: "https://lh3.googleusercontent.com/aida-public/AB6AXuCCEBSgyvWhWcUFD0_sFk0Z5fn365kbumbRqrv0GfIZuJzhpIr_eUJNWeIBGOl3ilIv7K2R5i4SFlgLPUmMLkSojNB61S0Y-tspj1SNDCxbSMr327OASz9b24mdOjMqUvq2GKMtN4H5b8Se1gxiKRGOkQKZgNqW7QE59DYjL4guWRIGz4azjNWxeGTr8iJBEJBzwMmY-49ETYtOTynhsY_SqQ6vOxPyxzNOP3VZLhi8Skrjr1D3s7TyorKUMJf9YiCbHAmFsV07M9c" },
      { name: "Priya Sharma", email: "priya@sundew.com", password: "password123", role: "HR Coordinator", avatar: "https://images.unsplash.com/photo-1494790108377-be9c29b29330?auto=format&fit=crop&q=80&w=200" }
    ];
    localStorage.setItem("hrms_registered_hr", JSON.stringify(users));
  } else {
...
function handleLogin(e) {
  ...
  const user = users.find(m => m.email === email && m.password === password);
...
function renderAssociateList() {
    ...
    associateListHtml += `
      <button onclick="selectAssociate('${associate.id}')" class="w-full flex items-center justify-between px-3 py-2 rounded-xl text-left text-sm transition-all focus:outline-none mb-1 ${activeClass}">
        <div class="flex items-center gap-3">
          ...
          <div class="truncate">
            <span class="block truncate">${associate.name}</span>
            <span class="block text-[10px] opacity-70 truncate">${associate.manager}</span>
          </div>
        </div>
  ...
  container.innerHTML = `
    <div class="mb-2">
      <div class="text-xs font-bold text-on-surface-variant uppercase tracking-wider px-3 mb-2 flex justify-between items-center">
        <span>Probation Associates</span>
        <span class="text-[10px] font-semibold text-secondary">${associates.length} total</span>
      </div>
      <div id="associates-section" class="space-y-1">
        ${associateListHtml || '<p class="px-3 text-xs text-on-surface-variant italic">No associates found.</p>'}
      </div>
    </div>
  `;
}
...
function updateLiveScores() {
  ...
  if (starContainer) {
    ...
    for(let i = 0; i < 5; i++) {
        if(i < stars) {
            starsHtml += `<span class="material-symbols-outlined text-[24px]" style="font-variation-settings: 'FILL' 1">star</span>`;
        } else {
            starsHtml += `<span class="material-symbols-outlined text-[24px]" style="font-variation-settings: 'FILL' 0">star</span>`;
        }
    }
...
function updateProbationAssessment(associate) {
  const currentAvg = (associate.currentEvaluation.tech + associate.currentEvaluation.learn + associate.currentEvaluation.adapt + associate.currentEvaluation.attitude) / 4;
  
  // Calculate average of history + current
  const allScores = [...associate.history.map(h => h.average), currentAvg];
...
function renderPerformanceCurves(associate) {
  ...
  const curAvg = (curTech + curLearn + curAdapt + curAttitude) / 4;
  ...
  // Render sub charts
  drawTrendSvg("chart-tech", points.map(p => p.tech), "var(--secondary)");
  drawTrendSvg("chart-learn", points.map(p => p.learn), "#15803d");
  drawTrendSvg("chart-adapt", points.map(p => p.adapt), "#f59e0b");
  drawTrendSvg("chart-attitude", points.map(p => p.attitude), "var(--error)");
...
function drawTrendSvg(id, values, strokeColor) {
  ...
  if (N === 1) {
    ...
    svg.innerHTML += `
      ...
      <text x="${width/2}" y="${height - 2}" font-size="6" text-anchor="middle" fill="var(--on-surface-variant)">Month 1</text>
    `;
    return;
  }
...
// Helper to draw big cumulative average score SVG with area gradient
function drawTotalAvgSvg(id, pointsData) {
  ...
  const areaPathStr = `${pathStr} L ${coords[N-1].x} ${height - paddingY} L ${coords[0].x} ${height - paddingY} Z`;
  
  svg.innerHTML += `
    <path d="${areaPathStr}" fill="url(#totalGrad)" />
    <path d="${pathStr}" fill="none" stroke="var(--secondary)" stroke-width="2.5" class="chart-line" stroke-linecap="round" stroke-linejoin="round" />
  `;
...
function triggerQuickAction(actionType) {
  ...
  const email = name.toLowerCase().replace(/\s+/g, '') + "@sundew.com";
  
  if (actionType === "message") {
    addNotification(`Messaging interface opened for ${name}`, "success");
    alert(`Mock messaging chat window initialized for ${name}.\n(HR ID: ${currentUser.name})`);
  } else if (actionType === "email") {
    window.location.href = `mailto:${email}?subject=Probation Feedback&body=Hi ${name},\n\nI would like to share some feedback regarding your current probation period.`;
